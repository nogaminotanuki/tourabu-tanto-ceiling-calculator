...
入力した御札だけで天井へ届く場合、大きな結果には「最初の天井までに必要な御札鍛刀回数」を表示します。複数種類の御札がある場合は、ポイントの高い御札から使う想定です。入力した御札をすべて使った場合の超過ポイントは、次周への持ち越しとして別に表示します。必要資源は最初の天井到達までの回数を基準に計算します。
