...
入力した御札だけで天井へ届く場合、大きな結果には「最初の天井までに必要な御札鍛刀回数」を表示します。複数種類の御札がある場合は、ポイントの高い御札から使う想定で、天井到達までに使う札の内訳も表示します。入力した御札をすべて使った場合の超過ポイントは、次周への持ち越しとして別に表示します。必要資源は最初の天井到達までの回数を基準に計算します。

最初の天井までの計算と、入力した御札をすべて使った場合の鍛刀回数・到達天井回数・持ち越しポイントは、画面上で分けて表示します。
