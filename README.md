...
「結果をコピー」から、計算結果とツールURLをテキストとしてコピーできます。

入力時に自動計算します。「再計算」ボタンは、自動計算が反映されない環境で同じ計算を再実行するための予備操作です。「現在P・御札をクリア」で、天井・レシピ設定を残したまま入力だけを0に戻せます。

現在ポイントの入力欄は、上下ボタンまたは矢印キーで5Pずつ増減します。天井ポイントの詳細設定は1Pずつ増減します。どちらも直接入力では任意の整数を使用できます。
...
札なしのみの回数は `ceil((天井 - 現在P) ÷ 5)`。御札を使っても天井へ届かない場合は、各札のポイントを先に加算し、残ったポイントを札なし鍛刀で補います。最終鍛刀回数は「使用する札の合計枚数＋追加の札なし鍛刀回数」です。

入力した御札だけで天井へ届く場合、大きな結果には「天井までに必要な御札鍛刀回数」を表示します。複数種類の御札がある場合は、ポイントの高い御札から使う想定で、天井到達までに使う札の内訳も表示します。入力した御札をすべて使った場合の超過ポイントは、次周への持ち越しとして別に表示します。必要資源は天井到達までの回数を基準に計算します。

天井までの計算と、入力した御札をすべて使った場合の鍛刀回数・到達天井回数・持ち越しポイントは、画面上で分けて表示します。
...
- 札アイコン：`assets` 内の同名PNGを差し替えます。

CSS・JavaScriptを更新した場合は、`index.html` の読み込みパス末尾にあるバージョン番号（例：`?v=1.0.1`）も更新すると、公開直後に古いファイルがブラウザへ残るのを防げます。
